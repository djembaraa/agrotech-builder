...
export async function POST(request, { params }) {
  ...
  try {
    ...
    const { data, error } = await supabase.from('comments').insert({
      post_id: id, user_id: user.id, content
    }).select('*, profiles(full_name, profile_photo_url)').single()
    if (error) return err(error.message)

    const admin = getSupabaseAdminClient()
    const { data: post } = await admin.from('posts').select('comments_count').eq('id', id).single()
    await admin.from('posts').update({ comments_count: (post?.comments_count || 0) + 1 }).eq('id', id)
